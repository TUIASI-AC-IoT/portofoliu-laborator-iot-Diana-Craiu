...
#define CONFIG_EXAMPLE_FIRMWARE_UPGRADE_URL "https://192.168.250.166:5000/firmware.bin" 
...
esp_err_t get_version_from_server(char *version)
{
    esp_http_client_config_t config = {
        .url = "https://192.168.250.166:5000/versionNumber",
        .cert_pem = (char *)server_cert_pem_start,
        .cert_len = 1422,
        .event_handler = _http_event_handler,
        .keep_alive_enable = true,
    ...
    if (err == ESP_OK) {
        ...
        cJSON *root = cJSON_Parse(esp_http_client_read_response(client, buffer, content_length));
        if (root != NULL) {
            cJSON *version_json = cJSON_GetObjectItem(root, "version");
            if (version_json != NULL) {
                strncpy(version, version_json->valuestring, 50); 
            }
            cJSON_Delete(root);
        }
    }
...
{
    char current_version[50] = "0.1.11"; 
    char server_version[50];

    // obtin vers de la server
    if (get_version_from_server(server_version) == ESP_OK) {
        if (strcmp(current_version, server_version) < 0) {
            ESP_LOGI(TAG, "New version is available. Downloading firmware...");
            ota_task(NULL);
        } else {
            ESP_LOGI(TAG, "Current version is already updated.");
        }
    } else {
        ESP_LOGE(TAG, "Error");
    }
