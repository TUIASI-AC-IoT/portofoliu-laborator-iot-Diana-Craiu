...
esp_err_t post_handler(httpd_req_t *req)
{
    ...
    buf_len = httpd_req_get_hdr_value_len(req, "ssid-exemplu-1") + 1;
    if (buf_len > 1) {
        buf = malloc(buf_len);
        if (httpd_req_get_hdr_value_str(req, "ssid-exemplu-1", buf, buf_len) == ESP_OK) {
            ESP_LOGI(TAG, "Found header => ssid-exemplu-1: %s", buf);
        }
        free(buf);
    }
        return ESP_FAIL;
